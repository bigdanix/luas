...
local drawingFontsEnum = {
	...
	[1] = Font.new(getcustomasset("Proggy.json"), Enum.FontWeight.Regular),
	[2] = Font.new(getcustomasset("VerdanaBold.json"), Enum.FontWeight.Regular),
	[3] = Font.new(getcustomasset("Tahoma.json"), Enum.FontWeight.Regular)
}
-- function
...
function DrawingLib.new(drawingType)
	...
	elseif drawingType == "Text" then
		local textObj = ({
			Text = "",
			Font = DrawingLib.Fonts.Verdana, -- Default Font
			Size = 0,
		...
		return setmetatable(table.create(0), {
			__newindex = function(_, index, value)
				...
				elseif index == "Font" then
					value = math.clamp(value, 0, 3) -- CORRECTED CLAMP
					local newFont = getFontFromIndex(value)
					if newFont then textLabel.FontFace = newFont end
